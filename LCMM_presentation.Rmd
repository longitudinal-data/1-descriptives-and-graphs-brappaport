...
library(nnet) ##need this to do multinomial logistic regression
...
ppwave_model4 <- postprob(wave_model4,c(0.7,0.8,0.9)) #for the model with 4 classes
ppwave_model4[1]
...
The package 'lcmm' uses the plot function (or you can use the plot.lcmm function). Can choose what to plot by the "which" argument. Can plot: residuals, posterior probabilities, fit (or the actual class trajectories), link function, and others for other types of GMMs.
```{r LCMM default easy plots}
plot.lcmm
plot(wave_model1,which="fit",var.time="wave",bty="n")
plot(wave_model2,which="fit",var.time="wave",bty="n")
...
#use facets in ggplot to separate Classes for eaach predictor
#and plot Odds on X axis with CI around them (if they don't include 0, then they are significant)
```
...
```
Now the 2 class model is the best fit and neither class is inordinately small.

Paper detailing how these can be used for clinical research (Nagin & Odgers, 2010): https://www.ncbi.nlm.nih.gov/pubmed/20192788